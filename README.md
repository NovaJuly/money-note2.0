...
- **自定义时间范围**：快捷选择本周、本月、本年或任意日期区间
- 导出为 **Excel / JSON / CSV** 三种格式，一键下载
...
- `GET /api/records` – 获取记录列表（支持 `startDate` / `endDate` 过滤）
...
- [ ] **AI 智能记账**：接入 LLM，支持自然语言输入自动生成记录
- [ ] **智能分类建议**：根据备注和金额通过模型自动推荐分类
- [ ] **消费趋势预测**：基于历史数据预测下月支出
- [ ] 离线记录缓存与自动同步
- [ ] 深色模式
- [ ] 移动端适配优化
